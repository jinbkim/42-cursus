...
float	center_x, center_y, r;  // 그려질 원의 중심의 x좌표, y좌표, 그려질 원의 반지름
...
void	draw_circle(void)  // 원에 문자 채우기
{
	...
	while (++y < zone_h)
	{
		...
		while (++x < zone_w)
		{
			d = sqrtf(powf(center_x-x, 2) + powf(center_y-y, 2));  // 원의 중심에서 해당 좌표까지의 거리
			if (r < d)  // 반지름보다 원의 중심에서 해당 좌표까지의 거리가 더 크면
				continue ;
			else if (mode=='C' || r-d<1.0000000)  // 꽉채우는 원 이거나 텅빈 원일때 테두리거나
...
int		main(int argc, char **argv)
{
	...
	while (0 < (ret=fscanf(file, "%c %f %f %f %c\n", &mode, &center_x, &center_y, &r, &c)))
	{
		...
		if ((mode!='C' && mode!='c') || r<=0 || ret!=5)
			return (put_error(ERR2));  // 에러코드2와 개행을 출력하고, 1을 반환하며 프로그램 종료
		draw_circle();  // 읽은 정보를 바탕으로 직사각형에 문자 채우기
	}
	while (++i < zone_h)
	{
